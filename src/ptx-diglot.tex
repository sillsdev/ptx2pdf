...
\def\diglotLeft{% 
 ...
 \else%
   \ifdim\dp\excessL>0pt
     ...
     \global\setbox\partialL=\vtop{\joinboxes{\partialL}{\excessL}{2e}{0}}%
   \fi
   ...
   \ifdim\TempDim<-0.3\baselineskip% Only split for significant amounts...
     ...
     \ifdim\dimen1>\baselineskip % was 0pt
       ...
       \ifdim\TempDim>0pt % It split something off..
         \trace{D}{spare material to excessL (\the\ht0, \the\dp0, \the\ht\partialL,\the\ht\excessL,\the\dp\partialL,\the\dp\excessL}%
         %\showbox\excessL%
...
% \LeftBox and \partialL are empty
  ...
  \trace{D}{av: \the\availht, remaing:\the\TempDim, bs:\the\baselineskip}
...
%the depth is not recoverable we add a kern to remove the orignial depth.
 ...
 \ifdim\pr@vdepth>\baselineskip \trace{D}{Looks like this is already a vtop}\pr@vdepth=0pt
 \fi%
 %\bgroup
 \setbox0=\copy#1\setbox1=\vtop{\unvbox 0\setbox2=\lastbox}%Work on a copy so we don't break stuff.
 \trace{D}{pd:\the\dp2, ph=\the\ht2, d:\the\pr@vdepth}%
...
\def\upd@tep@rtial{%
  ...
  %
  \traceNum{d}{Adding to page: LB:\the\ht\LeftBox+\the\dp\LeftBox RB:\the\ht\RightBox+\the\dp\RightBox}
  \ifVisTrace
    \doVisTrace{L}
  \fi
