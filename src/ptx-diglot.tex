...
% \LeftBox and \partialL are empty
  ...
  \ifnum\tmppenalty=10000 %
    \tmppenalty=0 %
  \fi
  \trace{H}{digloleftTrial: b:\botmark, f:\firstmark, tr:\topRmark, br:\botRmark}
  \edef\t@st{\botmark}%
  \ifx\t@st\empty%
...
\newdimen\cumulativeDelta % Sum of \chunkDelta values
\cumulativeDelta=0pt
...
\def\upd@tep@rtial{%Actually add stuff to the page.
  ...
  \global\advance\cumulativeDelta by \chunkDelta
    ...
    \ifdim\dp\partialL>0pt %
      ...
      \ifdim\dp\partialL>\availht%
        ...
        \ifx\t@st\empty\else
          ...
          \ifuseLeftMarks
            \gdef\nextp@gefirstmark{\splitfirstmark}%
            \trace{H}{Setting bottom mark to \splitbotmark}%
            \xdef\p@gebotmark{\splitbotmark}%
          \fi
        \fi
        \ifvoid\partialL%
...
\def\@writep@ge {%
  ...
  \trace{D}{PFM:\p@gefirstmark PBM:\p@gebotmark}%
...
\def\diglotgr@db@x#1{%
 \vbox{\gr@db@@x{#1}\penalty 10000}%
 \penalty 10000
}
\def\testmarker{\hbox to 0pt{\vrule height 7pt depth 0pt width 0.5pt \kern-0.5pt}\message{_}}
%
\addtoendhooks{\ifdim\cumulativeDelta=0pt\else\message{Overall column delta \the\cumulativeDelta\space(-ve means left longer)}\fi}
\addtoinithooks{\let\gridb@x\diglotgr@db@x}
