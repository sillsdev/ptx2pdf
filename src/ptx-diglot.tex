...
% \LeftBox and \partialL are empty
  ...
  \edef\t@st{\botmark}%
  \ifx\t@st\empty%
    \trace{H}{Bottom mark was empty}% Presumably others not als
  \else%
    \edef\t@st{\firstmarks1}%
    \ifx\t@st\oldRmark\else %Dont use \firstmark if it's been used by Right!
      \ifx\t@st\topmarks1\else % or it's a carry-over,
        \ifx\topLmark\empty
          \trace{H}{Setting topLmark to \firstmarks1}%
          \xdef\topLmark{\firstmarks1}% remember first, if not already set
        \fi
        \ifuseLeftMarks%
          \edef\t@st{\p@gefirstmark}%
          \ifx\t@st\empty%
            \trace{H}{Setting (default) pagefirstmark to \firstmarks1}%
