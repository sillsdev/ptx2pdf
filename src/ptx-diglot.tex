...
\def\diglotRightExtra{%
  ...
  \trace{D}{diglotRightExtra, \the\outputpenalty, (\the\ht255+\the\dp255) (\the\ht\excessR+\the\dp\excessR) dc:\the\deadcycles }%
  \makevtop{255}%
  \tmppenalty=\outputpenalty %
  \ifnum\tmppenalty=10000 %
    \tmppenalty=0 %
...
% if not all material was set, hold stuff in \partialR and use \diglotRightExtra to put that into \excessR
  \trace{D}{diglotRightTrial, \the\holdinginserts, \the\outputpenalty, 
...
\def\diglotRight{%
 ...
 \traceNum{d}{diglotRight \the\outputpenalty, \the\savedpenalty, \the\Rchunkpenalty, (\ifnum\holdinginserts=0 Not\fi holding)}%
 \tmppenalty=\outputpenalty %
  % tempoarily split and see if there are marks in this text
  \bgroup\setbox0=\copy255 \setbox1=\vsplit0 to \maxdimen\egroup
   \edef\t@mp{\splitbotmark}
...
\outer\def\lefttext{%
  \trace{d}{lefttext ilp=\the\interlinepenalty}%
...
\outer\def\righttext{\trace{d}{righttext pL:\the\dp\partialL, xL:\the\dp\excessL, pR:\the\dp\partialR}\ifhe@dings\endhe@dings\fi
...
\def\nolefttext{\trace{D}{nolefttext}\global\savedpenalty=\lastpenalty\n@xtc@mmand
  \par\penalty-10001
...
\def\setboth{%
  ...
  \trace{D}{setboth \the\outputpenalty, \the\savedpenalty, \the\Rchunkpenalty,  \the\Rboxpenalty
