...
\def\LRspecific{VerticalSpaceFactor,LineSpacingFactor,FontSizeUnit,regular,bold,italic,bolditalic,SpaceStretchFactor,SpaceShrinkFactor}%,RTL  too, but it can't be treated this way.
\def\LRspecific{Regular,regular, bold,Bold,Italic,BoldItalic}%,RTL  too, but it can't be treated this way.
...
\def\pr@cessSpecific{\x@\pr@cessSp@cific \LRspecific,\E}

\def\sp@cificSide#1{\edef\sfx{\ifdiglot\ifdiglotL L\else R\fi\else @rig\fi}\MSG{sS #1:\sfx}\x@\let\csname#1\x@\endcsname\csname #1\sfx\endcsname}
\def\SSsp@cificSide#1{\x@\let\csname#1\x@\endcsname\csname #1\ds@ffix\endcsname} %Stylesheet version of above.
\def\s@veSpecific#1{\x@\let\csname#1@rig\x@\endcsname\csname #1\endcsname\ch@ckLR{#1}{L}\ch@ckLR{#1}{R}}
\def\ch@ckLR#1#2{\x@\let\t@mp=\csname#1#2\endcsname\ifx\t@mp\relax\ifdiglot\MSG{Side-specific #1#2 not defined, global #1}\x@\let\csname#1#2\x@\endcsname\csname #1@rig\endcsname\fi}
...
\def\saveLRspecific{}
\def\getLRspecific{}% For use with style definitions, etc.
\def\SSsetLRspecific{}% for use in stylesheet
