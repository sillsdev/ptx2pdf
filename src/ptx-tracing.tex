...
% | dmcl | Diglot module: complex layout
% | dmcr | Diglot module: complex recursive splitting and other in-depth details
% | dmcs | Diglot module: complex scoring
% | e | Extended(sidebar)
% | eg | Sidebar gridding 
% | eb | Extended borders
...
% | t | Tables and table of contents / things
% | tim | Timing
% | T | Triggers
...
%
% Magic command to build tracelist. from above. in vim, go to colon in the next line and type "ay$@a
% :/| *a *|/+1,.! awk -F\| '(NF==3){b=$2;gsub("[ \t]","",b);A=A b ","}{print}END{print "%\n\\def\\tracelist{" A "}\n"}'
\def\tracelist{A,b,B,C,c,ch,cov,d,dP,D,De,Df,Dh,Ds,dm,dmp,dmc,dmcr,dmcs,e,eg,eb,ebi,eh,et,f,fp,F,Fe,g,G,gI,h,hv,H,i,imt,I,j,m,mx,M,ma,n,nS,o,oh,orn,p,pt,P,per,s,sa,sb,spv,ss,sP,sc,sC,sh,sk,sko,S,t,tim,T,u,V,v,x,}
...
\def\y@s{y}
\def\tracing#1{\x@\xdef\csname tr@cemode-#1\endcsname{\y@s}\let\trace=\tr@cerfast \x@\let\csname tr@ce-#1\endcsname\tr@ceout}%
\def\untracing#1{\x@\xdef\csname tr@cemode-#1\endcsname{}\x@\let\csname tr@ce-#1\endcsname\notr@cer}%
\def\notr@cer#1#2{\relax}%
\untracing{a} % Fallback is nothing happens

%Make registered tracing codes call tr@ce-a by default 
\def\RegisterTrace#1{\def\tmpa{a}\def\tmp{#1}\ifx\tmp\tmpa\else\message{#1}\x@\def\csname tr@ce-#1\endcsname{\csname tr@ce-a\endcsname}\fi}
\def\trdefaults#1,#2\E{\RegisterTrace{#1}\def\tmp{#2}\ifx\tmp\empty\else \trdefaults#2\E\fi}
\message{Tracing routines registered for:}\x@\trdefaults\tracelist\E
\def\FastTracing{\let\trace\tr@cerfast}
\def\AllowAllTracing{\let\trace\tr@cer}

\addtoendhooks{\ifx\trace\tr@cer\ifx\tracemoan\empty\message{No unregistered trace flags. \string\FastTracing\space can be used}\else\message{*** Unregistered trace flags: \tracemoan. Don't use \string\FastTracing}\fi\fi}

\def\tracemoan{}
%\def\tr@cer#1#2{%
%    \ifcsname tr@ce-#1\endcsname\csname tr@ce-#1\endcsname{#1}{#2}\else\csname tr@ce-a\endcsname{#1}{#2}\fi}
%\def\tr@cer#1#2{%
    %\ifcsname tr@ce-#1\endcsname\csname tr@ce-#1\endcsname{#1}{#2}\else\edef\tracemoan{\tracemoan,#1}\csname tr@ce-a\endcsname{#1}{#2}\fi}
\def\tr@cer#1{%
    \ifcsname tr@ce-#1\endcsname \x@\let\x@\tr@cetmp\csname tr@ce-#1\endcsname \else \edef\tracemoan{\tracemoan,#1}\x@\let\x@\tr@cetmp\csname tr@ce-a\endcsname\x@\let\csname tr@ce-#1\endcsname\tr@cetmp\fi \tr@cetmp{#1}}
\def\tr@cerfast#1{\csname tr@ce-#1\endcsname{#1}} % Faster, but no fallback

\def\tr@ceout#1#2{%
    \immediate\write-1{#1> #2}}%
\def\traceNum#1#2{\global\advance\TRACEcount by 1\trace{#1}{+\the\TRACEcount: #2}}
\def\notr@ceout#1#2{\relax}%
\let\trace=\notr@cer % Initial value of \trace
\let\@@tr@cer=\tr@cer % What \trace gets (re)defined as by \trace
...
\def\tracetimset#1{\x@\ifx\csname tr@cemode-tim\endcsname\y@s \x@\xdef\csname @t-#1@\endcsname{\the\elapsedtime}\fi}
\def\tracetimcheck#1#2{\x@\ifx\csname tr@cemode-tim\endcsname\y@s \relax\edef\tim@check{\the\numexpr \elapsedtime - \csname @t-#1@\endcsname \relax}\tr@ceout{tim}{[#1] #2 \tim@check}\fi}
...
\def\DBGhvm@de#1{\message{\ifhmode h\fi\ifvmode v\fi mode #1}} % Useful tool to bisect code to answer: what on earth is triggering that vmode->hmode  transition?
%-tracing-support
