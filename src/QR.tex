...
\x@\def\csname MS:zqrcode\endcsname{%
  \bgroup\everypar{}%
    ...
    \fi
    \get@ttribute{vmin}%
    \ifx\attr@b\relax\else\def\QRminVersion{1}\fi
    \get@ttribute{unicode}%
    \ifx\attr@b\tru@ \temptrue \else\tempfalse\fi
    \get@ttribute{data}%
    \iftemp 
    ...
    \fi
    \QRcode
    \message{QR code is \the\ht\QRcodebox, req \the\QRbounding/\the\QRinside}%
    \get@ttribute{pgpos}%
    \dimen0=\QRbounding
    \advance\dimen0 -\QRinside
    \ifx\attr@b\relax |\kern-\dimen0\box\QRcodebox\kern\dimen0|\else
      \ifx\attr@b\loc@InText\box\QRcodebox
      \else
        \ifcsname ins-\attr@b\endcsname
          \x@\let\x@\p@cins\csname ins-\attr@b
          \picUsesInstrue
        \else
          \picUsesInsfalse
          \def\w@tsit{QRcode}%
