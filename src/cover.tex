...
\def\c@verbit#1#2{%
  ...
  \bgroup
    ...
    \esbe%
    \trace{cov}{Request  was (\the\PaperHeight x\the\PaperWidth)}%
    \trace{cov}{Wholepagepic is (\the\ht\wholepagepic+\the\dp\wholepagepic)x\the\wd\wholepagepic w \the\dimexpr-\b@dr@left@pad-\b@dr@right@pad\relax padding and hbleed of \the\dimexpr \lble@d + \rble@d\relax )}%
    \hskip -\dimexpr \b@dr@left@pad\relax
    \raise\dimexpr \bble@d\ifx\tp@ram\relax\else -\bpp@ram pt\fi\relax\hbox{\box\wholepagepic}%
...
\def\d@spine{{%
  \ifrotatespine
    ...
    \trace{cov}{Rotating spine}%
    \setbox0\hbox{\hskip\lble@d\c@verbit{spine}{\topskip=1pt \vfill\csname periphcontents=spine\endcsname\vfill}\hskip\rble@d}%
...
\def\m@kecover{%
  ...
  \iftemp
    ...
    \ifnum\interactionmode=2
      \showbox0
    \fi
    \trace{cov}{ship@utcoverinsert{cover}{}{\coverXbleed}{\coverYbleed}{\csname cover-y-spine\endcsname}{\csname cover-bleed\endcsname} box is (\the\ht0+\the\dp0)x\the\wd0}%
