...
\def\c@verbit#1#2{%
  ...
  \bgroup
    \def\t@stsz{1}%
    %\def\t@stsz{.33}%
    \PaperHeight=\the\dimexpr \t@stsz\dimexpr\csname cover-Y-#1\endcsname \ifble@d +\tble@d+\bble@d\fi\relax\relax
    \PaperWidth=\the\dimexpr \t@stsz\dimexpr\csname cover-X-#1\endcsname \ifble@d +\rble@d+\lble@d\fi\relax\relax
    \hsize=\PaperWidth
    \esb\cat cover_#1\cat*#2
    \esbe%
    \bgroup\s@tc@tprefix{cover_#1}\getsbp@ram{borderbottom}\ifx\cp@ram\tr@e
      \getsbp@ram{borderbpadding}%
      \global\let\bpp@ram\p@ram
      \getsbp@ram{borderwidth}%
      \global\let\bwp@ram\p@ram
    \else
      \global\let\bpp@ram\relax%
      \gdef\bwp@ram{0}%
    \fi
    \egroup
    ...
    \ifhasb@rder
      \trace{cov}{Covr had border}%
       \raise\dimexpr \dp\wholepagepic \ifble@d +\bble@d\fi
            \ifx\bpp@ram\relax\else -\bpp@ram pt \fi
            \relax
...
\def\m@kecover{%
  ...
  \iftemp
    ...
    \cover@maths
    %\tracingassigns=0
    \setbox0\vbox{\topskip=0pt \lineskip=0pt%
      ...
      \c@verbit{whole}{\topskip=0pt \lineskip=0pt
        \vbox to 0pt{\csname periphcontents=wholecover\endcsname}%
    %    \tracingall
