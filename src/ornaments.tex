...
\def\RuleLineWidth #1\relax{\defp@ram{rulelinewidth}{#1}}
\def\RulePattern #1\relax{\defp@ram{rulepattern}{#1}}
...
\x@\def\csname drawzrule-ornaments\endcsname{%
  \getp@ram{rulepattern}{zrule}{\styst@k}%
  \ifx\p@ram\relax
    \def\p@ttern{88||-}%
  \else
    \let\p@ttern\p@ram
  \fi
  \getp@ram{rulelinewidth}{zrule}{\styst@k}%
  \ifx\p@ram\relax
  ...
  \def\ornYalign{.5}%
  \endgraf
  \bgroup \edef\rule@width{\the\dimexpr \rule@wid\hsize\relax}%
    \trace{m}{Rule will be \rule@width (hsize=\the\hsize)}%
  \hbox to \hsize{\ifx\rule@pos\@lignLeft\else\hfil\fi
    \hbox to \rule@width{\orn@SetStretch{\rule@width}{x}{}{\p@ttern}\Ornaments{\p@ttern}}%
