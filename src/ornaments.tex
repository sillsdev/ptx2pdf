...
\def\check@orn@dot#1.#2.#3\E{%
  \edef\d@t@tmp{#2}\ifx\d@t@tmp\empty
    \pdf@xunit=\csname orn@scale@#4@x\endcsname
    \pdf@yunit=\csname orn@scale@#4@y\endcsname
  \else
    \setorn@scale
...
\def\pdf@color@rgb@stroke#1#2#3{\sysprotocol@literal{#1 #2 #3 RG}}
\def\pdf@color@rgb@fill#1#2#3{\sysprotocol@literal{#1 #2 #3 rg}}
...
\LocalOrnament{-1}{10}{1}{\m 0.0 0.5 \l 10 0.5 \k }
\LocalOrnament{0}{10}{10}{ }
...
\def\orn@do@ornament{\bgroup%
    ...
    \setbox0\hbox to \dimen0{%
      ...
      \ifx\ornamentlinecol\empty\else
        \x@\pdf@color@rgb@stroke\ornamentlinecol
      \fi
      \ifx\ornamentfillcol\empty\else
        ...
        \else
          \ornamentfilltrue
          \x@\pdf@color@rgb@fill\ornamentfillcol%
        \fi
      \fi
      \ifornamentfill
...
\def\OrnamentBorder#1#2{%
  ...
  \edef\box@width{\dimexpr \wd#1 + #2 *\b@drsides+ \box@sep * \b@drsides\relax}%
  \edef\b@drsides{\numexpr \ifx\b@drtop\tr@e 1 +\fi \ifx\b@drbottom\tr@e 1+\fi 0\relax}
  \edef\box@height{\dimexpr \ht#1 + #2 *\b@drsides + \box@sep *\b@drsides\relax}%
  \ifx\b@drtop\tr@e
  ...
  \vbox to \box@height{\baselineskip=0pt\lineskip=0pt\hsize=\box@width\box\orn@bdr@top\hbox to \box@width{\box\orn@bdr@left\hss\hbox{\vbox{\kern\dimen2\box#1\kern\dimen2}}\hss\box\orn@bdr@right}\box\orn@bdr@bot}%
}
