...
\def\sc@letype@XY{%
  \ift@mp\else\message{Warning: Using xy or XY as part of a 2 part \escapechar OrnamentScaleRef makes no sense (given: \sc@ledbg)}%
  \fi
...
\x@\def\csname MS:zornament\endcsname{%
  \bgroup
    ...
    \get@ttribute{size}%
    \ifx\attr@b\relax
      \edef\ornamentheight{\the\dimexpr \c@rrfontsize\FontSizeUnit\relax}%
    \else
      \edef\ornamentheight{\attr@b}%
    \fi
    ...
    \else
      ...
      \relax
    \fi
    \message{end of zornament (\@@@result)}%
  \egroup
} 
...
\def\GraphicOrnament#1#2{% Convert an image #2 into local ornament #1
  \bgroup
    ...
    \else
      \x@\gdef\csname localornament@#1@code\endcsname{\hbox{\XeTeXPDFfile "#2" width \dimen0 height \dimen1 }}%
    \fi
  \egroup
}
