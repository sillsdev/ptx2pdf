...
\LocalOrnament{-1}{10}{10}{\m 0.0 5 \l 10 5 \k } % A line of equal lenght to a space
\LocalOrnament{-2}{20}{10}{\m 0.0 5 \l 20 5 \k } % A line twice as long as a space
\LocalOrnament{-3}{50}{10}{\m 0.0 5 \l 50 5 \k } % A line five times longer than as a space
...
% if =, no scaling will be applied, if it's not = or a number in formal N.N then a pre-set scale [set by setscale] will be applied 
...
\def\orn@SetStretch#1#2#3#4{%
  ...
  \begingroup
    ...
    \else
      ...
      \dimen5=\dimexpr \fill@stretch*32\relax%
    %  \loop \ifdim\dimen5<1pt
%	\multiply\dimen5 by 2
%	\multiply\dimen4 by 2
%	\repeat
      \count255=\dimen5 \divide\count255 by 65535 % Integer part only
      \MSG{\the\dimen4*32 / \the\count255}%
      \dimen4=\dimexpr (\dimen4*32) / \count255\relax%Scale by 32
...
\def\OrnamentBorder#1#2{%
  \MSG{OrnamentBorder}%
  \def\ornamentheight{#2}%
  \def\ornamentwidth{}%
  \def\ornYalign{1}%
  \edef\b@drsides{\numexpr \ifx\b@drleft\tr@e 1 +\fi \ifx\b@drright\tr@e 1+\fi 0\relax}%
  \edef\box@width{\dimexpr \wd#1 + #2 *\b@drsides+ \box@hsep * \b@drsides\relax}%%
  \edef\b@drsides{\numexpr \ifx\b@drtop\tr@e 1 +\fi \ifx\b@drbottom\tr@e 1+\fi 0\relax}%
  \edef\box@height{\dimexpr \ht#1 + #2 *\b@drsides + \box@vsep *\b@drsides\relax}%
  \ifx\b@drtop\tr@e
    \orn@SetStretch{\box@width}{x}{}{\patternT}\setbox\orn@bdr@top\hbox to \box@width{\Ornaments{\patternT}}%
  \else
    \setbox\orn@bdr@top\hbox{}%
  \fi
  \def\ornYalign{0}%
  \ifx\b@drbottom\tr@e
    \orn@SetStretch{\box@width}{x}{=}{\patternB}\setbox\orn@bdr@bot\hbox to \box@width{\Ornaments{\patternB}}%
  \else
    \setbox\orn@bdr@bot\hbox{}%
  \fi
  \MSG{#2 - \the\ht\orn@bdr@top -\the\dp\orn@bdr@top -\the\ht\orn@bdr@bot -\the\dp\orn@bdr@bot}%
  \dimen1=\dimexpr \box@height - \ht\orn@bdr@top -\dp\orn@bdr@top -\ht\orn@bdr@bot -\dp\orn@bdr@bot\relax
  \dimen2=0.5\dimexpr \dimen1 - \ht#1\relax
  %\def\ornamentheight{}
  %\def\ornamentwidth{#3}
  \message{Box side is \the\dimen1}%
  \tempfalse
  ...
  %\showboxdepth=10
  %\showbox\orn@bdr@right
  \setbox#1\vbox to \box@height{\baselineskip=0pt\lineskip=0pt\hsize=\box@width\box\orn@bdr@top\hbox to \box@width{\box\orn@bdr@left\hss\hbox{\vbox{\kern\dimen2\box#1\kern\dimen2}}\hss\box\orn@bdr@right}\box\orn@bdr@bot}%
}
...
\setsbp@ram{borderhpadding}{0.5ex}
\setsbp@ram{bordervpadding}{0.5ex}
\setsbp@ram{borderlinewidth}{1}

\def\DefaultOrnament{80}
\def\mkb@rder@ornaments#1{% Plugin for sidebars
  ...
  \getsbp@ram{borderhpadding}\let\box@hsep\cp@ram
  \getsbp@ram{bordervpadding}\let\box@vsep\cp@ram
  \getsbp@ram{borderlinewidth}\edef\ornamentlinewidth{\strip@pt{\dimexpr\cp@ram\FontSizeUnit\relax}}%
  \OrnamentBorder{#1}{\b@drwidth\FontSizeUnit}%
  \message{ORNAMENTS box \the\ht#1+\the\dp#1 * \the\wd#1}%
  %\showbox#1\copy #1 
}
