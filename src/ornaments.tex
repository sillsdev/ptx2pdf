...
\newif\ifboxorn\boxornfalse % Set to true to surround all ornaments with boxes.
...
\LocalOrnament{-1}{10}{10}{\m 0.0 5 \l 10 5 \k } % A centred line of equal length to a space
      ...
      \ifXformOrnaments
        ...
        \ifpdf@ornament@repeat\else
          \ifholdXobjects\else\ifvoid\ornXobjects\else
            \trace{orn}{Emitting Xobj definitions: \ornXFid@list (gt:\the\currentgrouptype, gd:\the\currentgrouplevel)}%
...
%#2 orientation
%#3 Stretch/mult
%#4 Size {=|[a-z]|N.NN}  If a number (1.0 is valid, 2. is not) then the item's size will be this much times the normal calculated size.
...
\def\@ornament@parse#1|#2|#3|#4|#5|#6\E{%#6 is scrap
  ...
  \ifx\tmp\empty\else
	...
	\x@\chk@cplx#3\E %Split off first code
        ...
        \ifx\tmpa\ptn@sim
	  \pdf@ornament@repeattrue
	  \edef\orn@repeat@class{\tmpb}% Variable, range set elsewhere
	\else 
...
% =V	- Variable V (unchanged from result.)
