  ...
  \ifx\tmp\empty\else
	...
	\x@\chk@cplx#3\E %Split off first code
        ...
        \ifx\tmpb\tmpu\else % variable needs to be  upper case 
          \MSG{\tmpb\space is lowercase! Treat as \tmpu}%
...
\def\prep@dims#1#2{%Prepare dimensions for calcsc@re
  \advance\dimen1 -\dimexpr \t@mp * #2 \relax % reduce ideal length
  \ifdim \t@mpstr>0pt
    \advance\dimen0 by -\dimexpr\dimexpr \t@mpstr * #2\relax / 2\relax % it's stretchy
  \fi
  \x@\edef\csname mult@class@num@#1\endcsname{\the\count255}%
  \ifdim \dimen0 =0pt
    \dimen0=-100sp
  \fi
  \MSG{remaining:\the\dimen1, shrink:\the\dimen0, \the\dimen2}% 
}
  
\def\calcsc@re{\bgroup \dimen4=\ifdim\dimen1<0pt 0.1\dimen0\else 0.1\dimen2 \fi\count255=\dimen4 
  \MSG{\the\dimen1 / \the\count255}% 
  \dimen4=\dimexpr 10\dimen1 / \count255\relax \count255=\dimen4
  \MSG{precheck score: \the\dimen4 (\the\count255)}%
  \let\D@\apply@checks\the\mult@classes
  \let\D@\m@lti@print\the\mult@classes
  \MSG{postcheckdscore: \the\dimen4 (\the\count255)}%
  \print@multi
  ...
  \ifpdf@ornament@repeat
    ...
    \else
      ...
      \ifpdf@ornament@stretch
        ...
        \x@\edef\csname mult@class@stretch@dim@\orn@repeat@class\endcsname{\the\dimexpr\tmp+\the\dimen\@@orndim\relax}%
      \fi
...
\def\multi@try#1,#2\E#3{\bgroup
	...
	\ifcsname mult@repeat@min@#1\endcsname
	  \ifnum \csname mult@repeat@min@#1\endcsname>\count255
	    ...
	    \MSG{ min for #1 (\the\count255) doesn't fit, but try it anyway (it might be shrinkable)}%
	  \fi
	\fi
      \else
	\count255=0
