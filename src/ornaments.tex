...
\def\sc@letype@XY{%
  ...
  \fi
  \x@\edef\csname \sc@lename x\endcsname{\the\dimexpr \b@drwidth\FontSizeUnit/ \@pgfornamentX\relax}%
  \x@\edef\csname \sc@lename y\endcsname{\the\dimexpr \b@drwidth\FontSizeUnit/ \@pgfornamentY\relax}%
}
\def\sc@letype@X{%
  \x@\edef\csname \sc@lename x\endcsname{\the\dimexpr \b@drwidth\FontSizeUnit/ \@pgfornamentX\relax}%
  \ift@mp
...
\def\orn@do@ornament{\bgroup%
    ...
    \ifXformOrnaments
      ...
      \edef\ornUUID{ornament-\ornamentlinewidth-\ornamentlinecol-\ifornamentfill \ornamentfillcol\else nofil\fi-\tpdf@xunit-\tpdf@yunit-\OrnamentsFamily\pdf@ornamentNum}%
      \edef\ornUUID{\x@\x@\x@\zap@space \x@\ornUUID\space \empty}%
    \else
      \edef\ornUUID{ornament-\ornamentlinewidth-\geom@xform-\ornamentlinecol-\ifornamentfill \ornamentfillcol\else nofil\fi-\the\pdf@xunit-\the\pdf@yunit-\OrnamentsFamily\pdf@ornamentNum}%
    \fi
    %\message{\ornUUID}%
    ...
    \else
      ...
      \ifXformOrnaments
        \edef\ornXFid{orn\pdf@ornamentNum-\the\ornXFids}%
      ...
      \ifXformOrnaments
        \global\setbox\ornXobjects\hbox{%\unhbox\ornXobjects %
          \edef\tmp{pdf:bxobj @\ornXFid\space width \the\dimen0 \noexpand\space height \the\dimen1 }%
    ...
    \iftemp\else 
      ...
      \ifnum 0<  \ifCacheOrnaments 1\fi\ifXformOrnaments 1\fi 0 
        ...
        \x@\xdef\csname \ornUUID-matrix\endcsname{\pdf@matrix}%
      \trace{orn}{dimens: \the\dimen0 \x@\space x \the\dimen1, 2:\the\dimen2, 3:\the\dimen3, 4:\the\dimen4, 5:\the\dimen5}%
      \fi
    \fi
...
\def\OrnamentBorder#1{%
  ...
  \setbox\b@rderbox\vbox to \b@rderbox@height{\baselineskip=0pt \lineskip=0pt \hsize=\b@rderbox@width
    \box\orn@bdr@top\hbox to \b@rderbox@width{\box\orn@bdr@left\hskip\border@l@pad\hss\hbox{\vbox to \dimen1{}}\hskip\border@r@pad\hss\box\orn@bdr@right\kern-\b@drwidth\FontSizeUnit}\baselineskip=\ht\orn@bdr@bot\box\orn@bdr@bot}%
  %\showbox#1%
...
\def\OrnamentScaleRef #1\relax{\initc@t\setsbp@ram{ornamentscaleref}{#1}}
...
\def\mkb@rder@ornaments{% Plugin for sidebars
  ...
  %\tracingassigns=1
  \getsbp@ram{ornamentscaleref}\ifx\cp@ram\relax\else\x@\p@rsescaleref \cp@ram,\E\fi
  %\tracingassigns=0
...
\def\GraphicOrnament#1#2{% Convert an image #2 into local ornament #1
  \bgroup
	...
	\edef\@@x{\the\dimexpr \pdf@xunit*\@pgfornamentX\relax}%
	\edef\@@y{\the\dimexpr \pdf@yunit*\@pgfornamentY\relax}%
