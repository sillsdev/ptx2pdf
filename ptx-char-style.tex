...
\def\ch@rstyle#1{\TRACE{ch@rstyle:#1}%
 ...
 \catcode32=12\relax % make <space> an "other" character, so it won't be skipped by \futurelet
...
\def\doch@rstyle{% here, \n@xt has been \let to the next character after the marker
 \catcode32=10\relax % reset <space> to act like a space again
 \if\n@xt*\let\n@xt\endch@rstyle % check for "*", if so then we need to end the style
 \else\let\n@xt\startch@rstyle\fi % else we need to start it
 \n@xt} % chain to the start or end macro

\catcode`\~=12 \lccode`\~=32 % we'll use \lowercase{~} when we need a category-12 space
\lowercase{% when \startch@rstyle is called, the following <space> has become category-12
...
%
% the \Superscript attribute isn't really supported everywhere, only for \v and footnote callers
%
\def\h@ndlesuperscript#1#2{% apply superscripting to #2, if specified by style #1
