...
class ViewModel:
    ...
    def generatePicList(self, booklist):
        ...
        if diglot:
            digmode = self.get("fcb_diglotPicListSources")
            secprjid = self.get("fcb_diglotSecProject")
            if secprjid is not None:
                if digmode.startswith("Sec") and secprjid != "":
                    # print("in Sec")
                    self.doError("Sorry!", secondary="This feature has not been implemented yet!")
                    return
                    # prjid = secprjid
                elif digmode.startswith("Both"):
                    # print("in Both")
                    self.doError("Sorry!", secondary="This feature has not been implemented yet!")
                    # self.doError("Sorry!", secondary="This feature of blending illustrations from both Pri and Sec projects has not been implemented yet!")
                    return
                else: # if "Pri"
                    # print("in Else (pri)")
                    prjid = self.get("fcb_project")
        ...
        for bk in booklist:
            ...
            if diglot:
                piclist.append("% Source for DIGLOT Illustrations: {} = {}\n".format(self.get("fcb_diglotPicListSources"),prjid))
