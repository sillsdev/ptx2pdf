    ...
    def generateNProcPicLists(self, bks, outdir, processor, isTemp=False):
        ...
        for bk in bks:
            if diglotPrinter is not None:
                picinfos.update(("{}L {}".format(bk, k), v) for k,v in self.getFigures(bk, sfmonly=True).items())
                diglotPics.update(("{}R {}".format(bk, k), v) for k,v in diglotPrinter.getFigures(bk, sfmonly=True).items())
            else:
                picinfos.update(("{} {}".format(bk, k), v) for k,v in self.getFigures(bk, sfmonly=True).items())
