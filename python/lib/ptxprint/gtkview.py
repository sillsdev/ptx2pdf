...
""".format(self.getvar("country", ""), self.getvar("languagename",  ""), \
    ...
    def onShareItClicked(self, btn):
        ...
        # parlocs_files = self.find_parlocs_files()
        # print(f"{len(parlocs_files)=}")
        # exception_tally = defaultdict(int)
        # for file in parlocs_files[:50]:
            # print(file)
            # self.testparlocs = Paragraphs()
            # self.testparlocs.readParlocs(file, rtl=False)
            # try:
                # continue
            # except Exception as e:
                # exception_type = type(e).__name__
                # exdtl = f"{exception_type}: {e}"
                # print(f"{exdtl}: {file}")
                # exception_tally[exdtl] += 1

        # print("\nException Summary:")
        # for exception_type, count in exception_tally.items():
            # print(f"{exception_type}: {count}")

        # print(f"\nTotal number of exceptions: {sum(exception_tally.values())} from {len(parlocs_files)} parlocs files")

    # def find_parlocs_files(self):
        # root_dirs = [
            # r"C:/My Paratext 9 Projects",
            # r"C:/Users/mark-/Downloads"
        # ]
        # target_dir = os.path.join("local", "ptxprint")
        # parlocs_files = []
                    ...
                    # for subdir in os.listdir(dirpath):
                        ...
                        # if os.path.isdir(full_subdir):
                            # for filename in os.listdir(full_subdir):
                                # if filename.endswith(".parlocs"):
                                    # parlocs_files.append(os.path.join(full_subdir, filename))

        # print(f"Found {len(parlocs_files)} .parlocs files")

        # Write the list to a text file
        # output_file = r"C:/ptx2pdf/ListOfAllParLocsFiles.txt"
        # os.makedirs(os.path.dirname(output_file), exist_ok=True)  # Ensure output directory exists
        # with open(output_file, 'w') as f:
            # for file in parlocs_files:
                # f.write(f"{file}\n")

        # print(f"List of .parlocs files written to {output_file}")

        # return parlocs_files
