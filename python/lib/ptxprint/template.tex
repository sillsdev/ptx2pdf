...
% This still needs to be fixed. Back in Info, we need to calculate if linemin or linemax == 0, 
% and if so, don't output '0pt' (as it causes it to crash)
\baselineskip={paragraph/linespacing}pt {paragraph/varlinespacing} plus {paragraph/linemax}pt minus {paragraph/linemin}pt
...
% This next set of options can only be active if "Use PT macros" is False = "" (as they use the new ptxplus macros)
{project/ifuseptmacros}{document/marginalverses}\input ptxplus-marginalverses.tex
{project/ifuseptmacros}\endbooknoeject{project/ifstarthalfpage}
