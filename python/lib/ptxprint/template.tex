...
{document/ifdiglot}\stylesheet{R_}{{{document/diglotcfgrpath}/ptxprint.sty}} % Right side (secondary) styles
...
%%%%% Covers
\catcode`\@=11
\lowercase{{\def\tmp{{{cover/coverartbleed}mm}}}}   % Art Bleed dimension (should eventually apply to all edge-hughhkkkging figures)
\def\bad{{nonemm}} %unfortunately, it gives 'none' sometimes
\ifx\tmp\bad\else
  \let\figbleed\tmp
\fi
