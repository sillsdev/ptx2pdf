...
texpertOptions = {
    ...
    "squashgridbox":      O("squashgrid", "LAY", True, "", _("No Top Space"), _("Don’t insert space above headings at the start of a column")),
...
#    "versehyphen":        O("vhyphen", "CVS", True, "", _("Margin Verse Hyphens"), _("In marginal verses, do we insert a hyphen between verse ranges?")),
    ...
    "NoHangVerseNumberOne": O("nohangvone", "CVS", True, "", _("Don't hang verse one beside a chapter"), _("Hanging verses into a chapter number can result in clases, depending on the after-chapter space.")),
    "HangVA":             O("hangva", "CVS", False, "", _("Alternate verse numbers also hang"), _("If there is an alternative verse and the current verse is hanging, does the alternate hang?")),
    ...
    "IndentAtChapter":    O("chapindent", "BDY", False, "", _("Allow Indent Para with Cutouts"), _("Allow indented paragraphs at chapter start with cutouts")),
    "IndentAfterHeading": O("afterheadindent", "BDY", True, "", _("Allow Indent Para After Heading"), _("Allow indented paragraphs following a heading")),
    ...
    "usesysfonts":        O("usesysfonts", "FNT", True, "", _("Use system fonts"), _("Use fonts from your system as well as your project")),
    ...
    "MidPageFootnotes":   O("midnotes", "NTS", False, "", _("Mid-Page Footnotes"), _("Should footnotes go before a single-double column transition")),
    "FinalNotesDown":     O("fnotesdown", "NTS", False, "", _("Final Page Notes Down"), _("Push notes on the final page to the bottom of the page")),
    ...
    "notesEachBook":      O("neachbook", "NTS", True, "", _("Endnotes at Each Book"), _("Output endnotes at the end of each book")),
    "newparnotes":      O("newparnotes", "NTS", False, "", _("Enhanced paragraphed footnotes"), _("Paragraphed footnotes which allow \\fp to work and also the ifparnotes... controls below. Causes some spacing differences compared to old-style.")),
    "parnoteskillprevdepth": O("parntkillpd", "NTS", True, "", _("Remove depth above paragraphed notes"),_("Kern away (kill) the glyph/font depth of note types before paragraphed notes, using the value of \\prevdepth")),
    "parnoteskilldepth":   O("parntkilldepth", "NTS", False, "", _("Remove depth after paragraphed notes"), _("Kill the depth of (the last row of) paragraphed note styles. Paragraphed notes will use the margin as their baseline.")),
    "parnotesruletopskip":   O("parntruletopskip", "NTS", False, "", _("Use grid not font/glyph height at rule"),_("If true, the footnote rule is positioned relative to the hypothetical baseline above the topmost footnote, rather than relative to the glyph/font height.")),
    "parnotesmidtopskip":   O("parntmidtopskip", "NTS", True, "", _("Use grid not font/glyph height between notes"),_("If true, then the \\InterNoteSpace is relative to the hypothetical baseline above the topmost footnote, rather than the glyph/font height. \\InterNoteSpace is triggered between note styles.")),
    "FootNoteGlyphMetrics": O("fnxugm", "NTS", False, "", _("Use glyph height not font height in footnotes"),_("Does the note use the font's descender design parameter, or the actual bottom of the glyph. If the former (and depth is not killed), then the bottom of notes have a common baseline across pages. If the latter, then the glyphs sit as close to the bottom margin as they can, which may permit an extra line of text.")),
    "lastnoteinterlinepenalty":  O("lastnoteinterlinep", "NTS", (10000, -9999, 10000, 10, 100, 0), "{0}={1}", _("Last Note: interlinepenalty"), _("Penalty for breaking between lines of the last footnote.")),
    ...
    "VisTrace":           O("vistrace", "DIG", False, "", _("Show Diglot Trace Marks"), _("Insert visible trace marks in diglot output")),
    "VisTraceExtra":     O("vistracex", "DIG", False, "", _("Extra Trace Mark Info"), _("Add extra information to diglot trace marks")),
    ...
    "figlocleft":         O("figleft", "PIC", True, "", _("Default Figures Top Left"), _("Default figure positions to top left")),
    "CaptionRefFirst":    O("captreffirst", "PIC", False, "", _("Ref Before Caption"), _("Output reference before caption")),
    "CaptionFirst":       O("captfirst", "PIC", False, "", _("Show Caption Before Image"), _("Output caption before image")),
    ...
    "DropActions":        O("noactions", "PDF", False, "", _("No PDF Bookmarks"), _("Don’t output PDF bookmarks")),
    "Actions":            O("actions", "PDF", True, "", _("Allow Active Links in PDF"), _("Make links active inside PDF")),
    "refbookmarks":       O("refbkmks", "PDF", False, "", _("Use Book Codes in PDF Bookmarks"), _("Use book codes instead of book names in bookmarks")),
    "NoTransparency":     O("notransparent", "PDF", False, "", _("Disable Transparency in PDF"), _("Disable transparency output in PDF")),

    "UnderlineSpaces":    O("underlnsp", "OTH", True, "", _("Underline Spaces"), _("Underline spaces in underlined runs")),
    "TOCthreetab":        O("tocthreetab", "OTH", True, "", _("Use \\toc3 for Tab Text"), _("Use \\toc3 for tab text if no \\zthumbtab")),
    ...
    "MarkTriggerPoints":  O("mktrigpts", "OTH", False, "", _("Mark Trigger Points"), _("Display trigger points in output")),
