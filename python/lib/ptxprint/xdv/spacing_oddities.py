...
class SpacingOddities(XDViPositionedReader):
    ...
    def xglyphs(self, opcode, parm, data):
        ...
        if not math.isclose((self.h-prev_pos[0]), 0, abs_tol = self.ctol):
            ...
            if self.new_line_needed(prev_pos):
                self.lines.append(Line(prev_pos[1],self.curr_ref, self.curr_fontsize))
    ...
    def xfontdef(self, opcode, parm, data):
        ...
        if self.new_line_needed((self.h, self.v)):
            ...
            self.lines[-1].change_font((self.h, self.v), self.curr_fontsize)
        self.cursor = (self.h,self.v)
        return (k, font)
...
class Line: 
    ...
    def change_font(self, pos, fontsize):
        self.curr_fontsize = fontsize
        self.gcs.append(GlyphContent(pos, self.curr_fontsize))

    def add_glyphs(self, prevpos, w): # in points
        if len(self.gcs) == 0:
            self.gcs.append(GlyphContent(prevpos, self.curr_fontsize))
    ...
    def has_badspace(self, threshold = 4):
        # threshold in ems
        if len(self.gcs) > 1:
            fontsize = self.gcs[0].font_size
            maxspace = fontsize*threshold
            for i in range(0, len(self.gcs)-1):
                if self.gcs[i].font_size != fontsize:
                    fontsize = self.gcs[0].font_size
                    maxspace = fontsize*threshold
                if (self.gcs[i+1].h_start - (self.gcs[i].h_start + self.gcs[i].width)) > maxspace:
                    return True
        return False
...
def main():
    ...
    for l in reader.lines:
        if l.has_badspace():
            print(f"Ref {l.ref} at v={l.v_start} has badspace.")
            badlines += 1
