  ...
  \def\@v@ ##1 {\gdef\v@rse{##1}%
   ...
   \ifc@ncelfirstverse\else
     ...
     \h@ndlesuperscript{v}{%
       ...
       \@V~\printv@rse\@V*}%
     \csname after-v\endcsname\kern\AfterVerseSpace \fi
   \egroup
   \m@rkverse
   \the\v@rsehooks
   \gdef\reference{\ch@pter:\v@rse}%
   \ifc@ncelfirstverse\else
     \nobreak\hskip1sp % let \x detect that there's space here
   \fi}
  \def\v{\leavevmode
...
\newdimen\AfterVerseSpace \AfterVerseSpace=2pt
...
\gdef\@ddspecialhooks{% there are a few special USFM markers that we give "magic" properties
 ...
 \let\@CL=\cl%
 \def\cl{\bgroup\obeylines\@cl}% \cl gets stored as chapter label and changes \c format
 \def\@cl ##1^^M{\gdef\ch@plabel{##1}\egroup}%
 \let\@ID=\id%
 \def\id{\bgroup\obeylines\@id}% \id gets stored and printed in the margin with cropmarks
