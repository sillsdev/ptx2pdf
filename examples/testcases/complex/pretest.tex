...
\def\try#1{\count255=1 \max=0pt
  ...
  \edef\tmp{\the\max|\the\dimen0|\csname #1bot\endcsname}\tryid|\tmp\par
...
\def\p@rsetry#1|#2|#3\E{%
  \def\p@rsemx{#1}\def\p@rseunbal{#2}\def\p@rsev{#3}%
}

\def\unbalVcost{20} % cost per verse
\def\unbalLcost{50} % cost per pt
\def\WScost{0.5} % cost per pt of whitespace
\def\maxv{0}
\def\minv{999}
\def\sc@re#1{%
  ...
  \else
    \ifx\nxt@ptions\empty
      ...
      \edef\tmp{\path|\minv|\maxv|\the\dimen0|\the\tryscore}\MSG{\tmp}\tmp\par
