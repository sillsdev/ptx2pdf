...
\def\try#1{\count255=1 \max=0pt
  ...
  \x@\edef\csname #1list\endcsname{\ifx\tmp\relax\else\tmp\fi\tryid,}
  \edef\tmp{\the\max|\the\dimen0|\dims|\csname #1bot\endcsname}\tryid|\tmp\par
...
\def\suboptions#1,#2\E{%
  ...
  \bgroup
    ...
    \def\psep{,}
    \MSG{scoring suboption #1 : \path (#2)}%
    \sc@re{#1}
  \egroup%
  \ifx\s@tmp\empty\let\n@xt\cstackrelax
  \else\let\n@xt\suboptions\fi
...
\def\p@rsetry#1|#2|#3|#4\E{%
  \def\p@rsemx{#1}\def\p@rseunbal{#2}\def\p@rsev{#4}%
}

\def\unbalVcost{200} % cost per verse
\def\unbalLcost{5} % cost per pt
\def\WScost{0.2} % cost per pt of whitespace
\def\maxv{0}
\def\minv{999}
\def\sc@re#1{%
  \x@\let\x@\tmp\csname#1\endcsname
  \x@\p@rsetry\tmp\E
  \advance\tryscore by \unbalLcost\dimexpr\p@rseunbal\relax\relax
  \ifnum\p@rsev>\maxv \let\maxv\p@rsev \fi
  \ifnum\p@rsev<\minv \let\minv\p@rsev \fi
  %\MSG{\p@rsev: min,max = \minv,\maxv}%
  ...
  \else
    \ifx\nxt@ptions\empty
      ...
      \advance\tryscore by \dimexpr \dimen0
      \edef\tmp{Path \path: \the\tryscore|\the\dimen0|\minv|\maxv}\MSG{\tmp}\tmp\par
...
\cols=2 \dimen1=\ht\Lbox \divide\dimen1 by \cols
\try{L}
\try{R}
