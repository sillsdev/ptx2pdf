...
\def\try#1{\count255=1 \max=0pt
  ...
  \loop
    ...
    %\iflua\directlua{measure_ins(\the\count255)}\else
      \me@sureins{#1}{\the\count255}%
    %\fi
    \edef\ins@rtl@st{\ifx\ins@rtl@st\empty\else\ins@rtl@st+\fi\insht}%
  \ifnum \count255<\cols
  ...
  \edef\tmp{\the\max|\the\numexpr \dimexpr \dimen0\relax /\baselineskip\relax|\csname #1bot\endcsname|\ins@rtl@st}\trace{Cx}{\tryid|\tmp}\par
  \x@\let\csname \tryid\endcsname\tmp
}
\newif\ifminitrialfull

\def\@me@sureins#1#2{
  \ifminitrialfull
    \holdinginserts= 0
    \s@veinserts{minitrial}%
    \x@\s@venotes\x@{\@g@tndstat{#1}}{1}%
  \else 
    \holdinginserts= 1 
  \fi
  \trace{Cxt}{Running \ifminitrialfull full \else quick \fi trial for #1 #2}%
  \unvbox#2
  \penalty\minitrialpenalty
  \relax
  \ifminitrialfull
    \r@storenotes{\@g@tndstat{#1}}{1}%
    \r@storeinserts{minitrial}
  \fi
}
...
\def\me@sureins#1#2{%
  ...
  \bgroup
    ...
    \minitrialfullfalse
    \@me@sureins{#1}{#2}%
    \ifminitrialfull\else
      \ifdim\minitrial@result>0pt
        \minitrialfulltrue
        \@me@sureins{#1}{#2}%
      \fi
    \fi
  \egroup
  \xdef\insht{\minitrial@result}%
  \trace{Cxt}{minitrial@result=\insht(\mintrialins@dims)}%
}

\def\me@sureinserts#1\E{%
  \ifdim\ht\csname #1\endcsname>0pt
    \xdef\mintrialins@dims{\ifx\mintrialins@dims\empty\else\mintrialins@dims,\fi#1:\the\ht\csname #1\endcsname}%
  \fi
}

\def\minitrial{%
  \xdef\minitrial@result{\the\dimexpr \premtht-\the\pagegoal\relax}%
  \setbox255\vbox{\unvbox255}
  \global\let\mintrialins@dims\empty%
  \ifminitrialfull
    \ifdim\minitrial@result>0pt
    ...
    \fi
  \fi
  %\trace{Cxt}{minitrial: \the\ht255, \minitrial@result}%
}
...
\def\p@rsetry#1|#2|#3|#4\E{%
  \def\p@rsemx{#1}\def\p@rseunbal{#2}\def\p@rsev{#3}\def\p@rsins{#4}%
}
