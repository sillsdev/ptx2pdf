\def\pluginlist{ornaments,cover}
\input "paratext2.tex"
\LogFontstrue
\catcode`\@=11
\def\hSEQ{\noexpand\horizSeq}
\def\SEQ{\noexpand\vertSeq}
\def\horizSeq{\the\orn@seq@num}
\def\vertSeq{\the\orn@seq@num}
\tracing{m}
\tracing{I}
\tracing{Fe}
\tracing{F}
\tracing{sC}
\tracing{s}
%\def\MapBorderTlabels{^^^^03b1,β,γ,δ,ε,ζ}
\pluginjustwarntrue
\def\ptxversion{0}
...
\tracing{sc}
\tracing{orn}
\def\OverThinRule{warnfix}
\def\OverThinCurve{warnfix}
%\tracing{sC}
\stylesheet{test.sty}
...
%\boxorntrue
%\nonstopmode
\def\tracingIF{\tracingifs=1}
\def\info{\dimen1=\prevdepth\message{bls:\the\baselineskip ld:\the\lastdepth, pd:\the\dimen1}}
\def\ornamentlinewidth{0.1}
\def\testornamentsv{
  \def\ornXalign{0}%
  \def\ornamentlinecol{0 0 0}
  \def\ornamentfillcol{0.9 0.4 0.4}
  \SwitchOrnamentsFamily{vectorian}
\xdef\ornamentheight{20pt}\xdef\ornamentwidth{}\xdef\ornamentmaxwidth{\the\dimexpr 0.7\hsize\relax}
\def\ornamentlinewidth{0.1}
\lineskip 1em
\count255=-3
\ornscaletrue
...
\def\ornloop{\loop\ifnum\count255<\max \relax
  \the\count255
  \vrule height 0.5pt width 1em
    \ifcsname orn@skip\the\count255\endcsname (skipped)\else
    \doOrnament{\the\count255}\fi
  \vrule height 0.5pt width 1em \hfil\penalty0\hfil
...
\tracingoutput=1
\newbox\myorn
\setbookhook{start}{all}{\setbox\myorn\hbox{\zornament|pattern="1|||" \*}}%showbox\myorn}
\sethook{start}{p}{\leavevmode \copy\myorn}% 
\hfuzz=500pt
\ptxfile{test.usfm}
\zNeedEvenPage
\makeatletter
...
\showboxdepth=999
\showboxbreadth=99
\message{OrnamentUnit \the\OrnamentUnit}%
\tracing{cov}
\eject
%\BookOpenLefttrue
%\RTLtrue
...
\boxornfalse
\unprepusfm
\x@\def\csname cover-hinge\endcsname{3mm}
\m@kecover
\bye
